/**
 * A-Z INGREDIENT INDEX — CURRENTLY OFF THE SITE.
 *
 * The page is kept intact but taken out of routing. The folder is named
 * `_a-z` rather than `a-z`: a leading underscore makes it a Next.js private
 * folder, excluded from the route tree, so nothing resolves at `/a-z` any
 * more. The file itself could not simply be commented out — the App Router
 * requires a default export from every `page.tsx` it finds, so a fully
 * commented file would fail the build rather than disappear.
 *
 * Also commented out, and needing to be put back alongside this:
 *   - `NAV` entries in `components/scientific/ScientificHeader.tsx` and
 *     `components/storefront/Header.tsx`
 *   - footer links in `components/scientific/ScientificFooter.tsx` and
 *     `components/storefront/Footer.tsx`
 *   - the `/a-z` entry in `app/sitemap.ts`
 *   - the "A-Z ingredient index" cross-links on the categories and functions
 *     pages, whose surrounding copy was also reworded away from the index
 *
 * TO RESTORE: rename this folder back to `a-z` and uncomment those six sites.
 */
